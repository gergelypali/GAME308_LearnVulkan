...
void VulkanRenderer::initVulkan() {
    DH->createInstance();
    instance = DH->GetInstance();

    std::cout << "initVulkan1" << std::endl;
    DH->setupDebugMessenger();
    std::cout << "initVulkan2" << std::endl;
    debugMessenger = DH->GetDebugMessenger();
    std::cout << "initVulkan3" << std::endl;
    DH->createSurface();
    std::cout << "initVulkan4" << std::endl;
    surface = DH->GetSurface();
    DH->pickPhysicalDevice();
    physicalDevice = DH->GetPhysicalDevice();
    DH->createLogicalDevice();
    device = DH->GetLogicalDevice();
    graphicsQueue = DH->GetGraphicsQueue();
    presentQueue = DH->GetPresentQueue();
    createSwapChain();
    createImageViews();
    createRenderPass();
...
void VulkanRenderer::createSwapChain() {
    ...
    QueueFamilyIndices indices = DH->findQueueFamilies(physicalDevice);
...
void VulkanRenderer::createDescriptorSetLayout() {
    ...
    VkDescriptorSetLayoutBinding globalLightingUboLayoutBinding{};
    globalLightingUboLayoutBinding.binding = 1;
    globalLightingUboLayoutBinding.descriptorCount = 1;
    globalLightingUboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    globalLightingUboLayoutBinding.pImmutableSamplers = nullptr;
    globalLightingUboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
...
void VulkanRenderer::CreateGraphicsPipeline() {
    auto vertShaderCode = readFile("shaders/phong_vert.spv");
    auto fragShaderCode = readFile("shaders/phong_frag.spv");
...
void VulkanRenderer::createCommandPool() {
    QueueFamilyIndices queueFamilyIndices = DH->findQueueFamilies(physicalDevice);
...
void VulkanRenderer::createVertexBuffer(const std::vector<Vertex>& modelVertices) {
...
void VulkanRenderer::createIndexBuffer(const std::vector<uint32_t>& modelIndices) {
...
void VulkanRenderer::createUniformBuffers() {
    ...
    bufferSize += sizeof(GlobalLightingUBO);
    bufferSize += sizeof(ModelUBO);
...
void VulkanRenderer::createDescriptorSets() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        VkDescriptorBufferInfo globalLightingBufferInfo{};
        globalLightingBufferInfo.buffer = uniformBuffers[i];
        globalLightingBufferInfo.offset = sizeof(CameraUBO);
        globalLightingBufferInfo.range = sizeof(GlobalLightingUBO);
        ...
        modelBufferInfo.buffer = uniformBuffers[i];
        modelBufferInfo.offset = sizeof(CameraUBO) + sizeof(GlobalLightingUBO);
        ...
        descriptorWrites[1].pBufferInfo = &globalLightingBufferInfo;
...
void VulkanRenderer::SetModelUBO(const Matrix4& model, const Matrix4& normal) {
    marioModelUbo.modelMatrix = model;
    marioModelUbo.normalMatrix = normal;
}

void VulkanRenderer::updateUniformBuffer(uint32_t currentImage) {
    void* data;
    vkMapMemory(device, uniformBuffersMemory[currentImage], 0, sizeof(CameraUBO), 0, &data);
    memcpy(data, &cameraUbo, sizeof(CameraUBO));
    vkUnmapMemory(device, uniformBuffersMemory[currentImage]);
    data = nullptr;
    vkMapMemory(device, uniformBuffersMemory[currentImage], sizeof(CameraUBO), sizeof(GlobalLightingUBO), 0, &data);
    memcpy(data, &globalLightingUbo, sizeof(GlobalLightingUBO));
    vkUnmapMemory(device, uniformBuffersMemory[currentImage]);
    data = nullptr;
    vkMapMemory(device, uniformBuffersMemory[currentImage], sizeof(GlobalLightingUBO) + sizeof(CameraUBO), sizeof(ModelUBO), 0, &data);
    memcpy(data, &marioModelUbo, sizeof(ModelUBO));
    vkUnmapMemory(device, uniformBuffersMemory[currentImage]);
}
