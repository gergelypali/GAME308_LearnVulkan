...
void DeviceHandler::CreateWindow(std::string name, int width, int height)
{
    SDL_Init(SDL_INIT_VIDEO);
    m_window = SDL_CreateWindow(name.c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE);
}
...
std::vector<const char*> DeviceHandler::getRequiredExtensions()
{
...
void DeviceHandler::setupDebugMessenger() {
    ...
    std::cout << "setupDebugMessenger3" << std::endl;
    if (result != VK_SUCCESS) {
        std::cout << "setupDebugMessenger4" << std::endl;
...
QueueFamilyIndices DeviceHandler::findQueueFamilies(VkPhysicalDevice device) {
